...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	стрДанныеДокумента 				= БазаИсточник__ПолучиДанныеДокумента(ИдВидаДокумента, ГУИДДокументаИсточникаСтрокой);
...
&НаСервере
Функция БазаИсточник__ПолучиДанныеДокумента(ИдВидаДокумента, ГУИДДокументаИсточникаСтрокой, ПрефиксБазы = "")
	...
	БазаИсточник_Запрос.Текст = "ВЫБРАТЬ "+ СписокПолейВыборки+ " ИЗ Документ." + ИдВидаДокумента + " КАК Источник ГДЕ Источник.Ссылка = &Ссылка";
	...
	"ВЫБРАТЬ
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Период) КАК Период,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Регистратор) КАК Регистратор,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.НомерСтроки) КАК НомерСтроки,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.ВидДвижения) КАК ВидДвижения,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Статус) КАК Статус,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Сессия) КАК Сессия,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Собственник) КАК Собственник,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.МестоХранения) КАК МестоХранения,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.НомернойЭлемент) КАК НомернойЭлемент,
	|ДвижениеНЭ.Количество КАК Количество,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Холдер) КАК Холдер
	|ИЗ
	|РегистрНакопления.ДвижениеНЭ КАК ДвижениеНЭ
	|ГДЕ
	|ДвижениеНЭ.Регистратор = &Ссылка";
	БазаИсточник_Запрос.УстановитьПараметр("Ссылка", БазаИсточник_ДокументСсылка);
	БазаИсточник_РезультатЗапроса = БазаИсточник_Запрос.Выполнить();
	Если Не БазаИсточник_РезультатЗапроса.Пустой() Тогда
		
		Для Каждого Кол Из БазаИсточник_РезультатЗапроса.Колонки Цикл
			НоваяКол = тз.Колонки.Добавить();
			НоваяКол.Имя = Кол.Имя;
		КонецЦикла;
		...
		Пока БИ_Выборка.Следующий() Цикл
			НовСтрока = тз.Добавить();
			Для Каждого Кол Из тз.Колонки Цикл
				НовСтрока[Кол.Имя] = БИ_Выборка[Кол.Имя]
			КонецЦикла;
		КонецЦикла;
	    		
	КонецЕсли;
	тз = ВставьВТаблицуЗначенийПервойСтрокойЗаголовкиКолонок(тз);
	Результат.Вставить("Регистр_Накопления_ДвижениеНЭ", тз);
	//</РН. ДвижениеНЭ>
	//<РН. ДвижениеНЭ>
	тз = Новый ТаблицаЗначений();		
	БазаИсточник_Запрос = БазаИсточник.NewObject("Запрос");
	...
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Сессия) КАК Сессия,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.НомернойЭлемент) КАК НомернойЭлемент,
	|ДвижениеНЭ.СуммаВсего КАК СуммаВсего,
	|ПРЕДСТАВЛЕНИЕ(ДвижениеНЭ.Холдер) КАК Холдер
	|ИЗ
	|РегистрНакопления.СебестоимостьНЭ КАК ДвижениеНЭ
	|ГДЕ
	|ДвижениеНЭ.Регистратор = &Ссылка";
	БазаИсточник_Запрос.УстановитьПараметр("Ссылка", БазаИсточник_ДокументСсылка);
	БазаИсточник_РезультатЗапроса = БазаИсточник_Запрос.Выполнить();
	...
	КонецЕсли;
	тз = ВставьВТаблицуЗначенийПервойСтрокойЗаголовкиКолонок(тз);
	Результат.Вставить("Регистр_Накопления_СебестоимостьНЭ", тз);
	//</РН. ДвижениеНЭ>
...
Функция ПолучиПараметрыСоединенияПоПрефиксуИБ(ПрефиксИБ)
	...
	Если ПрефиксИБ = "ОУ" Тогда
		ПараметрыСоединения = "Srvr=""MainAPPM"";Ref=""asc_oper"";";		
	КонецЕсли;
	Возврат  ПараметрыСоединения
КонецФункции

&НаКлиенте
Процедура ФормаИсточникаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ПрефиксИБ = "ОУ";
	ПараметрыСоединения = ПолучиПараметрыСоединенияПоПрефиксуИБ(ПрефиксИБ);
			
	Парам = Новый Структура();
	Парам.Вставить("ПараметрыСоединения"	, ПараметрыСоединения);
