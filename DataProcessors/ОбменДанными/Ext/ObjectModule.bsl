...
Процедура ДобавитьДокументРТУ(ТЗ, БазаИсточник);	
	...
	|	И Т.Номер = &НомерДок
	|	И Т.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&начДата, ДЕНЬ) И КОНЕЦПЕРИОДА(&конДата, ДЕНЬ)
	|	И Т.Организация = &Организация";
	Запрос.УстановитьПараметр("НомерДок", нНомерДок);
	Запрос.УстановитьПараметр("начДата", НачалоДня(ТЗ[0].ДатаДок));
	Запрос.УстановитьПараметр("конДата", КонецДня(ТЗ[0].ДатаДок));	Запрос.УстановитьПараметр("Организация", Организация);
	...
	БазаИсточник_ГУИД_ДокументаИсточника = БазаИсточник.NewObject("УникальныйИдентификатор", ТЗ[0].ДокументИсточник_ГУИД_Строкой);
	ИстДокумент = БазаИсточник.Документы["НЭРеализация"].ПолучитьСсылку(БазаИсточник_ГУИД_ДокументаИсточника);
	Если ИстДокумент = Неопределено Тогда
	...
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ";
	БазаИсточник_Запрос.УстановитьПараметр("Ссылка", ИстДокумент);
	БазаИсточник_РезультатЗапроса = БазаИсточник_Запрос.Выполнить();
	
	Если Не БазаИсточник_РезультатЗапроса.Пустой() Тогда
		ИстГрузополучатель 		= ИстДокумент.Получатель;
		АтрибутыКонтрагента 	= ПолучитьАтрибутыКонтрагента(БазаИсточник, ИстГрузополучатель);
	...
	//<Получение ссылки на документ-источник>
	БазаИсточник_ГУИД_ДокументаИсточника = БазаИсточник.NewObject("УникальныйИдентификатор", ТЗ[0].ДокументИсточник_ГУИД_Строкой);
	БазаИсточник_ДокументИсточникСсылка=БазаИсточник.Документы[(ТЗ[0].ТипДокументаИсточникаСтрокой)].ПолучитьСсылку(БазаИсточник_ГУИД_ДокументаИсточника);
	//</Получение ссылки на документ-источник>
	ЗарегистрироватьФактЗагрузкиВБазеИсточнике(БазаИсточник, БазаИсточник_ДокументИсточникСсылка);
КонецПроцедуры
