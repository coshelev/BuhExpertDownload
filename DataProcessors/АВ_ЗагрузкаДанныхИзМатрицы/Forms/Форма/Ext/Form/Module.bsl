	...
	Для Каждого СтрокаТЧ из ОтчетКомитенту Цикл
		...
		Иначе
			...
			Иначе	
				СтрокаТЧ.ДоговорКонтрагента = НайтиДоговор(СтрокаТЧ.Контрагент, 
														 СтрокаТЧ.ДоговорНомер, 
														 //<Кошелев_28.06.2022, =>, не искать по дате>
														 // СтрокаТЧ.ДоговорДата //=>
														 '00010101'
														 //</Кошелев_28.06.2022>
														  , 
			...
			Если СтрокаТЧ.Состояние = "РЕАЛИЗАЦИЯ" Тогда
				...
				Иначе	
					СтрокаТЧ_Товары.ДоговорКонтрагента = НайтиДоговор(СтрокаТЧ_Товары.Контрагент, 
															 ...
															 //<Кошелев_28.06.2022, =>, не искать по дате договора>
	...
	Для Каждого СтрокаТЧ из РеализацияАвтомобилей Цикл
		...
		Иначе
			...
			Иначе	  
				...
				СтрокаТЧ.ДоговорКонтрагента = НайтиДоговор(СтрокаТЧ.Контрагент, 
														 СтрокаТЧ.ДоговорНомер, 
														 //<Кошелев_07.06.2022_14-49, =>, Не передаем дату; Кошелев_05.09.2022_10-07, возвращаем передачу даты>	
														 СтрокаТЧ.ДоговорДата
														 //'00010101'
														 //</Кошелев_07.06.2022_14-49>
														  , 
														   СтрокаТЧ.ДоговорУИД, 
						   Перечисления.ВидыДоговоровКонтрагентов.СПокупателем,
						           СтруктураОрганизации.ПодразделениеВДоговореРеализация, 
...
Функция ЗагрузитьОтчетКомитенту(Параметр_Подразделение, Параметр_Склад)
	ЕстьОшибки = Ложь;
	Для Каждого СтрокаТЧ из ОтчетКомитенту Цикл  
		//<Кош, 08.04.2022,+, ошибка в рамках одной связки реализация - отчет комитенту>
		ЕстьОшибка = Ложь;
		//<>
	...
	#КонецОбласти
		...
		#Область ДОКУМЕНТ_Отчет_агента   
			...
			Если Не ЕстьОшибка и ( _ПерезаполнятьНайденное или не ЗначениеЗаполнено(СтрокаТч.Документ)) Тогда
		...
		Попытка 
			...
			Для Каждого СтрокаДвижения из ДвижениеДока цикл  
				...
				Если СтрокаДвижения.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120 Тогда
					...
					//<Кош, 19.04.2022, =>, уберем из СуммыБезНДС собственно НДС отдельной обработкой. Уберем ручную корректировку>
					//СтрокаДвижения.СуммаБезНДС = Окр(СтрокаДвижения.НДС/20*120,0);
					//ЕстьИсправленныеДвижения = Истина; =>
					
					//СуммасНДС = Окр(СтрокаДвижения.НДС/20*120,2);
					//СтрокаДвижения.СуммаБезНДС = СуммасНДС - СтрокаДвижения.НДС;
					//</Кош>
				КонецЕсли;
			КонецЦикла;   
...
Функция НайтиДоговор(Контрагент, НомерДоговора,ДатаДоговора, УИД_Строка, ВидДоговора, ПодразделениеВДоговоре, Подразделение = неопределено, ТочкаОформления = неопределено, ДоговорТип = неопределено, ВидРасчетовДоговора = неопределено)
		...
		Иначе	
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Если не (ВыборкаДетальныеЗаписи.Объект.Номер = номерДоговора и ВыборкаДетальныеЗаписи.Объект.Дата  = ДатаДоговора) Тогда
					...
					//<Кошелев_05092022_14-37, //, выводим только номер и дату>
	...
	КонецЕсли;
		...
		Если НомерДоговора<>неопределено  Тогда      //Не пустой договор
			...
			Иначе // Если не по подразделению
				...
				//<Кошелев_05092022_10-05, //, Не ищем договор по дате>
	...
	Если _СоздатьНенайденныеЭлементы Тогда
		...
		Если НомерДоговора<>неопределено Тогда
			...
			//<Кошелев_07.06.2022, =>Убрать из номера договора дату>
			//НовыйДоговор.Наименование = ?(ДоговорТип = неопределено,"",СокрЛП(ДоговорТип)+" ") + "№ "+НомерДоговора+ " от "+Формат(ДатаДоговора,"ДФ=dd.MM.yyyy"); //=>
			НовыйДоговор.Наименование = НомерДоговора+ ?(ЗначениеЗаполнено(ДатаДоговора)," от ","")+Формат(ДатаДоговора,"ДФ=dd.MM.yyyy"); 
			//</Кошелев_07.06.2022>
...
Процедура ЗаполнитьДокументВозвратТиУ(Док, СтрокаТЧ, СтруктураОрганизации, ЭтоЕнВД, ЭтоАгентскийДоговор = Ложь)
			...
			Для Каждого СтрокаТЧ_Номенклатура из СтрокаТЧ.Товары Цикл
				...
				Если ЭтоАгентскийДоговор Тогда                        
					НС.Номенклатура = СтрокаТЧ_Номенклатура.Автомобиль;   
					//<Кош, 19.04.2022, =>, замена счета с 41 на 41.01>
...
Процедура ЗаполнитьДокументОтчетКомитенту(Док, СтрокаТЧ)
	...
	//<Кош, +, 07042022_15_01>
	Док.СпособРасчетаКомиссионногоВознаграждения  	= ?(Не ЗначениеЗаполнено(Док.СпособРасчетаКомиссионногоВознаграждения), Перечисления.СпособыРасчетаКомиссионногоВознаграждения.ПроцентОтРазностиСуммПродажиИПоступления, Док.СпособРасчетаКомиссионногоВознаграждения);
    Док.ПроцентКомиссионногоВознаграждения			= ?(Не ЗначениеЗаполнено(Док.ПроцентКомиссионногоВознаграждения),100,Док.ПроцентКомиссионногоВознаграждения);
	Док.Субконто 									= Справочники.НоменклатурныеГруппы.НайтиПоНаименованию("Агентское вознаграждение (продажа автомобилей)");   
	Док.СчетУчетаНДСПоРеализации 					= ПланыСчетов.Хозрасчетный.Продажи_НДС;  
	Док.УдержатьВознаграждение 						= Истина;
	Док.СчетУчетаРасчетовСКонтрагентом 				= ПланыСчетов.Хозрасчетный.ПрочиеРасчетыСРазнымиДебиторамиИКредиторами;
	//</Кош>
...
//	Док = Документы.ОтчетКомиссионераОПродажах.СоздатьДокумент();
	...
	Для Каждого СтрокаТЧ_Номенклатура из СтрокаТЧ.Товары Цикл
		...
		//<Кош, 19042022, =>, Замена счета с 68.02 на 90.03>
		...
		//<Кош, 08042022, =>, Расчет суммы НДС вознаграждения от ставки НДС вознаграждения>  
